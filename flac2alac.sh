...
function _convert_flac2alac {
    ...
    local ARTFORMAT=$(metaflac --export-picture-to=- "${1}" \
                        | file -i -b - \
    ...
    if [ "${ARTFORMAT}" != "application/x-empty" ]
    then
...
do
    ...
    then
        IFS=$'\n'
        for one_filename in $(find "${filename}" -type f -iname "*.flac")
        do
            unset IFS
